...
impl Game for Pong {
    ...
    fn tick(&mut self, dt: f32, console: &mut Console<Self>) {
        ...
        if console.input.key_held(VirtualKeyCode::A) || console.input.key_held(VirtualKeyCode::Left) {
            self.player.paddle_pos -= 100.0 * dt;
        }

        if console.input.key_held(VirtualKeyCode::D) || console.input.key_held(VirtualKeyCode::Right)
        {
            self.player.paddle_pos += 100.0 * dt;
        }
        ...
        self.player.paddle_pos = clamp(
            ...
            10.0,
            console.graphics.height() - 15.0 -10.0,
        );

        // Set up the rectangles for the ball and paddles
        let ball_rect = Rect::new(self.ball_pos.x, self.ball_pos.y, 10.0, 10.0);

        let player_paddle_rect = Rect::new(self.player.paddle_pos, console.graphics.height() - 15.0 - 10.0 , 50.0, 10.0);
        ...
        let ai_paddles = vec![
            ...
            Rect::new(
            console.graphics.size.x as f32 - 75.0,
            self.ai.paddle_pos,
            20.0,
            10.0,
            ),
            ...
            Rect::new(
            50.0,
            self.ai.paddle_pos + 80.0,
            10.0,
            10.0,
            ),
            Rect::new(
            20.0,
            self.ai.paddle_pos + 60.0,
            10.0,
            10.0,
            ),
        ...
        for (i, ai_paddle_rect) in ai_paddles.iter().enumerate() {
        ...
        console.graphics.draw_rect(*ai_paddle_rect, 0xFF0000, true);
        ...
        console
            .graphics
            .draw_rect(player_paddle_rect, 0x00FF00, true);
